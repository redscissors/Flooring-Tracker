...
- *Errata 2026-09-02 (final review):* the planning measurement ran `unitOf` on the pricelist row alone; on the assembled entry the stock row's unit wins, so no `unit` moves. The derived change is `sizeText` on `US5000019` and `US5000044`, pinned by the stock-half test.
