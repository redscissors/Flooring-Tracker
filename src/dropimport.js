...
export function fileFormat({ sheets, pages, isPdf }) {
  ...
  if (isPdf) return isManningtonCartons(pages || []) ? "mannington" : isMirageChart(pages || []) ? "mirage-chart" : isTrueTouch(pages || []) ? "ovf-truetouch" : isInterfacePriceList(pages || []) ? "interface" : isSomersetPriceList(pages || []) ? "somerset" : "generic";
