...
test("normalizeSettings attaches pricing defaults and keeps stored values", () => {
  assert.deepEqual(normalizeSettings().pricing, { builderPct: 8, salePct: 10, sheogaMarkupPct: 40 });
  const s = normalizeSettings({ pricing: { builderPct: 12, salePct: 25, sheogaMarkupPct: 55 } });
  assert.deepEqual(s.pricing, { builderPct: 12, salePct: 25, sheogaMarkupPct: 55 });
});

test("serializeSettings persists pricing and round-trips", () => {
  const s = normalizeSettings({ pricing: { builderPct: 6 } });
  const out = serializeSettings(s);
  assert.deepEqual(out.pricing, { builderPct: 6, salePct: 10, sheogaMarkupPct: 40 });
  assert.deepEqual(normalizeSettings(out).pricing, { builderPct: 6, salePct: 10, sheogaMarkupPct: 40 });
});
