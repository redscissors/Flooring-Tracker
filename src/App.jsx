...
function orderLineCost(p, s, sell) {
  if (String(p.costSqft ?? "").trim() !== "") {
    ...
    if (p.type === "misc") { const PC = getPieceCarton(p); return csf * (PC ? PC.pieces : miscQty(p)); }
...
export function MobileProductRow({ p, settings, tv, onOpen, onPointerDown }) {
  ...
  const sf = p.qtyType === "sqft" ? num(p.qty) : 0;
  const tLine = tierPrice == null ? c.line : p.type === "misc" ? tierPrice * (c.PC ? c.PC.pieces : miscQty(p)) : c.C ? c.C.order * c.C.sf * tierPrice : sf * tierPrice;
...
export function MobileRowSheet({ p, areaName, canDelete, settings, stock, gFamilies, searchOrder, bookName, tv, onPatch, onPickStock, onDelete, onClose, qtyRef }) {
  ...
  const canSearch = stock.length > 0 || !!searchOrder;
  ...
  const pickGroutColor = (color) => { const it = gBook ? groutColorItem(stock, gBook, color) : null; const ck = gBook ? groutCaulkItem(stock, gBook, color) : null; onPatch({ grout: { ...p.grout, color, sku: it ? it.sku : "", caulkSku: ck ? ck.sku : "", caulkPrice: ck && ck.price != null ? String(ck.price) : "" } }); };
  const pickGroutProduct = (product) => { const book = settings.grouts[product]?.book || ""; const it = book && p.grout.color ? groutColorItem(stock, book, p.grout.color) : null; const ck = book && p.grout.color ? groutCaulkItem(stock, book, p.grout.color) : null; onPatch({ grout: { ...p.grout, product, sku: it ? it.sku : "", caulkSku: ck ? ck.sku : "", caulkPrice: ck && ck.price != null ? String(ck.price) : "" } }); };
  ...
  const addGrout = () => { if (groutDefault === p.grout.product) { onPatch({ grout: { ...p.grout, checked: true } }); return; } const book = settings.grouts[groutDefault]?.book || ""; const it = book && p.grout.color ? groutColorItem(stock, book, p.grout.color) : null; const ck = book && p.grout.color ? groutCaulkItem(stock, book, p.grout.color) : null; onPatch({ grout: { ...p.grout, checked: true, product: groutDefault, sku: it ? it.sku : "", caulkSku: ck ? ck.sku : "", caulkPrice: ck && ck.price != null ? String(ck.price) : "" } }); };
...
function VendorGroupCard({ group, groups, sheetSesid, progress, running, dragOverId, onRedownloadAll, onRedownloadSheet, onPatch, onDelete, onRemoveSheet, onMoveSheet, onStartDrag, inp }) {
  ...
  const unlockable = group.sheets.some((s) => sheetSesid(s));
...
export function VendorFetchPage({ settings, setSettings, onFiles, vendorPending, inp, lbl }) {
  ...
  const [drag, setDrag] = useState(null); // { sheet, fromId, x, y, overId }
  ...
  // pointerup handler off stale state.
  const setDragBoth = (v) => { dragRef.current = v; setDrag(v); };
  const startDrag = (e, sheet, fromId) => { e.preventDefault(); setDragBoth({ sheet, fromId, x: e.clientX, y: e.clientY, overId: null }); };
  useEffect(() => {
    if (!drag) return;
    const move = (e) => {
      const el = document.elementFromPoint(e.clientX, e.clientY);
      const card = el && el.closest ? el.closest("[data-vendor-group]") : null;
      setDragBoth({ ...dragRef.current, x: e.clientX, y: e.clientY, overId: card ? card.getAttribute("data-vendor-group") : null });
    };
    const up = () => { const d = dragRef.current; if (d && d.overId && d.overId !== d.fromId) moveSheet(d.sheet, d.fromId, d.overId); setDragBoth(null); };
    const key = (e) => { if (e.key === "Escape") setDragBoth(null); };
  ...
  return (
    <div>
      ...
      {drag && <div className="fixed z-50 pointer-events-none px-2 py-1 rounded-md bg-slate-800 text-white text-xs shadow-lg" style={{ left: drag.x + 10, top: drag.y + 10 }}>{entryFileName(drag.sheet)}</div>}
    </div>
  );
}
