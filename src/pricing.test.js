...
test("normPricing defaults builder 8 / sale 10 and clamps", () => {
  assert.deepEqual(normPricing(undefined), { builderPct: 8, salePct: 10 });
  assert.deepEqual(normPricing({ builderPct: 12, salePct: 15 }), { builderPct: 12, salePct: 15 });
  assert.deepEqual(normPricing({ builderPct: -5, salePct: 400 }), { builderPct: 0, salePct: 100 });
  assert.deepEqual(normPricing({ builderPct: "abc" }), { builderPct: 8, salePct: 10 });
});
