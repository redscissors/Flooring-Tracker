// Preview harness for the mobile product-box layout prototype (2026-07-23):
// Product/color across the top, Size · SKU · Coverage on row two, Price ·
// Needed · Order on row three, and the type pucks + "Price book" search moved
// into the pinned footer beside Done — which now rides above the on-screen
// keyboard (MobileSheet's visualViewport keyboard-follow). The rows, editor
// sheet, and full-screen search are the REAL MobileProductRow / MobileRowSheet
// / MobileSearchSheet from src/mobile.jsx, driven by a local fixture project +
// fixture price-book stock — no Supabase reads or writes. Based on the 021
// harness.
// Served by the vite dev server at
// /.scratch/046_mobile-product-box-preview/preview.html; never shipped.
...
import { stockPatch, groutFamilies } from "../../src/stock.js";
...
const normStockItem = (row) => normOrderItem({ ...(row.data || {}), sku: row.sku });
const STOCK = [
                ...
                {a.products.map((p, pi) => {
                  ...
                  const rowEditor = rowSheet?.pid === p.id ? (
                    <MobileRowSheet p={p} areaName={areaLabel(a, ai)} canDelete={a.products.length > 1 && !(rowBlank(p) && isAdder)}
                      settings={SETTINGS} stock={STOCK} groutStock={STOCK} stockReady bookStockReady isBookFam={() => false}
                      gFamilies={GFAMILIES} searchOrder={null} bookName={() => "special order"} tv={tv} notify={(m) => console.log(m)}
