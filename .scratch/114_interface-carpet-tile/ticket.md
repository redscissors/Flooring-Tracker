...
- The trim-as-area advisory flags 11 styles whose names carry design words
  ...
  carpet; mark reviewed once in the book.
- "Touch Of Timber Emea" — EMEA loses its caps to the word-caser. Cosmetic.
- The colorway "color book": blocked on Interface's item/color list export
  from the rep (see docs/pricebook/interface.md). When it arrives, grow the
  importer the Mirage joined-file way (ADR 0025 rule 7) to one row per
  style+color with real Interface item numbers.
